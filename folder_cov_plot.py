...
covay_m = mJK.covmatense(data_energy_ord, mJK.meanense(data_energy_ord))
...
mJK.matrix_plot(ax, dataordirrep, covay_m, label_all=True)

ax.set_xticklabels([str(dataordirrep[i]) for i in ax.get_xticks()], rotation=45, ha='left')

mJK.plt.savefig(f"{datafolder}/covar.pdf", transparent=True, bbox_inches='tight')
...
eig_val_data = mJK.np.sort(mJK.np.linalg.eigvals(covay_m))
...
ax.semilogy()
ax.grid(ls=':',lw=0.6)
ax.set_title(f'{datafolder} Covariance matrix eigenvalues')

mJK.plt.savefig(f"{datafolder}/covar_svd.pdf", transparent=True, bbox_inches='tight')

#plot percentage error
fig,ax = mJK.plt.subplots(1,1,num=3,figsize=(8,3))
label_df.plot(x="x_labels", y="data_err", ax=ax, ls='', marker='o', mfc='w', label="error", color='k')
...
mJK.plt.savefig(f"{datafolder}/err_val.pdf", transparent=True, bbox_inches='tight')
