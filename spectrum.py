...
def read_ini_file_BMA(filename):
    """ Read a mel_list file with BMA error and return a dictionary per irrep """
    ...
    with open(filename, 'r') as f:
        for line in f:
            ...
            data['irre'] = elems[2]
            data['lvl_num'] = int(elems[3])
            data['file_name'] = elems[4]
            data['BMA_error'] = float(elems[5])
...
def write_ini_file(filename, levelsinirrep):
    """ Write an name.ini file from a dictionary with irrep keys
    elems are a list of dictionaries with each having level info
    """

    with open(filename, 'w') as f:
        for irrep in levelsinirrep:
            level_list = levelsinirrep[irrep]
            for data in level_list:
                line = f"{data['vol']} {vec2label(data['mom'])} {data['irre']} {data['lvl_num']} {data['file_name']}"
                f.write(line + "\n")

    return f"Wrote ini file {filename}"
...
def label_state_refmom(state_dict):
    """ return a string labeling a level dictionary with ref mom """

    # make sure it is reference momentum
    momentum = np.copy(state_dict['mom'])
    momentum.sort()

    return f"V{state_dict['vol']}_d{vec2label(momentum)}_{state_dict['irre']}_#{state_dict['lvl_num']}"